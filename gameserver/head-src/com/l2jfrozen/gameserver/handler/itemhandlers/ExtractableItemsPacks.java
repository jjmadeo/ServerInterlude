...
import com.l2jfrozen.gameserver.datatables.csv.ExtractablePacksItemsData;
 ...
 */
public class ExtractableItemsPacks implements IItemHandler
{
	...
	public void doExtract(final L2PlayableInstance playable, final L2ItemInstance item, int count)
	{
		...
		if (count > item.getCount())
			return;
		while (count-- > 0) {
			L2ExtractableItem exitem = ExtractablePacksItemsData.getInstance().getExtractableItem(itemID);
			if (exitem == null)
				return;
	...
	{
		return ExtractablePacksItemsData.getInstance().itemIDs();
	}
}