...
import com.l2jfrozen.gameserver.handler.itemhandlers.ExtractableItemsPacks;
 ...
 * @author Nick
 */
public class ExtractablePackByPassHandler implements ICustomByPassHandler
{
	protected static final Logger LOGGER = Logger.getLogger(ExtractablePackByPassHandler.class);
	...
	public void handleCommand(final String command, final L2PcInstance player, final String parameters)
	{
		try
		{
			...
			if (ih == null || !(ih instanceof ExtractableItemsPacks))
				return;
			if (command.compareTo(_IDS[0]) == 0)
			{
				((ExtractableItemsPacks) ih).doExtract(player, item, 1);
			}
			else if (command.compareTo(_IDS[1]) == 0)
			{
				((ExtractableItemsPacks) ih).doExtract(player, item, item.getCount());
			}
		}
